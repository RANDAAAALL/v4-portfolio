    ...
    <section aria-labelledby="page-title">
      <div className="journal-list">
        {(Object.keys(journalPosts) as (keyof typeof journalPosts)[]).map(category => {
          ...
          return (
            <Link key={category} href={`/journal/${category}`} className="journal-entry">
              ...
              {preview && <div className="journal-preview"><Image src={preview.src} alt={formatCategoryName(category)} fill sizes="(max-width: 639px) calc(100vw - 48px), 268px" className="object-cover" /></div>}
            </Link>
          );
        })}
      </div>
